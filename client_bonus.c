...
/*   Updated: 2022/11/26 20:25:35 by cnascime         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	confirmation(int thumbsup);

// Insert the PID of the server in the first argument.
// Insert the message in the second argument.
// The message will be sent one character at a time, null character included.
// The server will print the message and hopefully send a confirmation.
// The client will print the confirmation and exit.
...
{
	if (thumbsup == SIGUSR2)
		write(1, "Recebido pelo servidor! :-)\n", 28);
	exit(0);
}
